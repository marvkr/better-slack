  ...
  protocol.handle('thumbnail', async (request) => {
    try {
      ...
      if (cached && cached.mtime === mtime) {
        return new Response(new Uint8Array(cached.data.buffer, cached.data.byteOffset, cached.data.byteLength) as any, {
          headers: {
      ...
      return new Response(new Uint8Array(data.buffer, data.byteOffset, data.byteLength) as any, {
        headers: {
          'Content-Type': 'image/png',
          'Cache-Control': 'max-age=3600',
        },
